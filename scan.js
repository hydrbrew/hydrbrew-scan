(function () {
    ...
    let totalScans = 0; // Initialize to 0, will be updated by fetchInitialCount()
...
async function fetchInitialCount() {
    // Setting the Origin header explicitly to bypass browser CORS block
    ...
    const res = await fetch(`${SUPABASE_URL}/rest/v1/rpc/get_total_scans`, {
        ...
        headers: {
            ...
            // Merging the required Origin header
            ...originHeader 
        }
    });
    ...
    if (res.ok) {
        ...
        console.error("Failed to fetch initial scan count:", res.status);
    }
}
...
async function fetchInitialCount() {
    ...
    // ... rest of the function remains the same ...
        ...
        if (res.ok) {
            const d = await res.json();
            // The result is an array containing the total count value from the function
            const rawValue = d?.[0]?.get_total_scans;
            if (rawValue) {
                totalScans = parseInt(rawValue, 10);
            }
    ...
    async function init() {
        console.log("Initialization complete. Fetching live scan count."); // Final log message
        await fetchInitialCount(); // <-- Fetch the real count before starting
    ...
    // Line 129: Call the async initialization function
    init();
